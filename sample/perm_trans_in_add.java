 ...
 */
public class perm_trans_in_add {
    Parent perm_trans_in_add_root;
    Scene perm_trans_in_add_scene;

    public void add_perm_trans_in() {
        try {
            perm_trans_in_add_root = FXMLLoader.load(getClass().getResource("perm_trans_in_add_gui.fxml"));
        } catch (IOException e) {
            System.out.println(e);
        }

        perm_trans_in_add_scene = new Scene(perm_trans_in_add_root, 1070, 500);
        perm_trans_in_add_scene.getStylesheets().add(getClass().getResource("style.css").toExternalForm());

        Main.stage.setScene(perm_trans_in_add_scene);
