 ...
 */
public class perm_trans_in_view_controller implements Initializable{
    @FXML
    private TableView<perm_trans_in_tabular> perm_trans_in_table;
    @FXML
    private TableColumn<perm_trans_in_tabular, Integer> card_no_col;
    @FXML
    private TableColumn<perm_trans_in_tabular, String> first_name_col, last_name_col, trans_from_col, date_in_col;
    @FXML
    ...
    Connection conn;
    ObservableList<perm_trans_in_tabular> perm_trans_in_data;
    ResultSet rs;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        perm_trans_in_data = FXCollections.observableArrayList();

        try {
            conn = db.java_db();
            select_query = "SELECT * FROM permanent_transfer_in;";
            rs = conn.createStatement().executeQuery(select_query);

            while (rs.next()){
                perm_trans_in_data.add(new perm_trans_in_tabular(rs.getInt("card_no"), rs.getString("first_name"), rs.getString("last_name"), rs.getString("transferred_from"), rs.getString("date_transferred_in")));
            }
        } catch (SQLException ex){
            Logger.getLogger(temp_trans_in_view_controller.class.getName()).log(Level.SEVERE, null, ex);
        }

        perm_trans_in_table.setItems(perm_trans_in_data);
        //setting a selection model to allow selection of only one item at a time
        perm_trans_in_table.getSelectionModel().setSelectionMode(SelectionMode.SINGLE);
    }
    ...
    public void delete() {
        ...
        pos = perm_trans_in_table.getSelectionModel().getSelectedCells().get(0);
        x = pos.getRow();
        //item here is the table view type
        item = perm_trans_in_table.getItems().get(x);
        ...
        try {
            ...
            if (confirm_delete){
                 ...
                 */
                del_query = "DELETE FROM permanent_transfer_in WHERE card_no = ?";
                ...
                perm_trans_in_data = perm_trans_in_table.getItems();
                getRow = perm_trans_in_table.getSelectionModel().getSelectedItems();
                getRow.forEach(perm_trans_in_data::remove);
            }
        } catch (Exception e) {
